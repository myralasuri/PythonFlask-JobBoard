...
def review(employer_id):
    if request.method == 'POST':
        ...
        nowdata= datetime.datetime.now().strftime("%m/%d/%Y")
        res = execute_sql('INSERT INTO review (review, rating, title, date, status, employer_id) VALUES (?, ?, ?, ?, ?, ?)',[review, rating, title, nowdata, status, employer_id], commit=True)
        print(res)
